    ...
    public static void LoadAllCards(string filePath)
    {
        ...
        foreach (var cardNode in cardsArray)
        {
            ...
            if (cardNode["PowerCardEffectType"] != null)
            {
                ...
                allCards[cardId] = new PowerCard(cardId, name, element, value, color, description, effectType);
            }
            else
            {
                allCards[cardId] = new RegularCard(cardId, name, element, value, color, description);
            }
        }
        AllCards = allCards;
    }
